...
import { Building2, FileText, Target, DollarSign, Coins, ChevronDown, ChevronUp } from 'lucide-react';

export interface BountyMeetingInfo {
  projectName: string;
  projectLogo: string;
...
interface BountyInfoCardProps {
  bounty: BountyMeetingInfo;
  defaultCollapsed?: boolean;
}

export function BountyInfoCard({ bounty, defaultCollapsed = false }: BountyInfoCardProps) {
    ...
    <Card className="p-3 space-y-2 bg-white/10 backdrop-blur-sm border-white/20 shadow-lg">
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h3 className="text-base font-bold text-white truncate">{bounty.projectName}</h3>
          </div>
        </div>
        {/* 折叠/展开图标 */}
      ...
      {!isCollapsed && (
        <div className="space-y-2 pt-2">
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <Target className="w-3 h-3 text-blue-400/80 flex-shrink-0" />
              <p className="text-[10px] font-semibold text-white/60 uppercase tracking-wide">
                Bounty Requirements
              </p>
            </div>
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <DollarSign className="w-3 h-3 text-green-400/80 flex-shrink-0" />
              <p className="text-[10px] font-semibold text-white/60 uppercase tracking-wide">
                Reward Structure
              </p>
            </div>
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-white/90 leading-relaxed pl-5 font-semibold">
              {bounty.bountyAmount}
            </p>
          </div>
        </div>
